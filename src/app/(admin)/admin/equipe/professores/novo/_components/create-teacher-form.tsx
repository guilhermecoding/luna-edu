...
import { createTeacherAction, promoteAdminAction } from "../actions";
import { toast } from "sonner";
import { Controller, useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { createTeacherSchema, promoteAdminSchema, type CreateTeacherData, type CreateTeacherInput, type PromoteAdminInput } from "../schema";
...
export default function CreateTeacherForm({ admins }: { admins: AdminOption[] }) {
    const router = useRouter();
    const [mode, setMode] = useState<"new" | "existing">("new");

    const formNew = useForm<CreateTeacherInput, unknown, CreateTeacherData>({
        ...
        defaultValues: {
            ...
            systemRole: "FULL_ACCESS", // Mesmo para professores podemos definir o escopo de acesso ao portal admin se necessário
        },
    });

    const formExisting = useForm<PromoteAdminInput>({
    ...
    const onSubmitNew = async (data: CreateTeacherData) => {
        clearErrorsNew("root");
        ...
        const result = await createTeacherAction(cleanData);
    ...
    const onSubmitExisting = async (data: PromoteAdminInput) => {
        clearErrorsExisting("root");
        const result = await promoteAdminAction(data);
                    ...
                    {admins.length === 0 ? (
                        <div className="p-8 flex flex-col items-center justify-center gap-2 border-2 border-dashed border-surface-border rounded-xl bg-surface/30 text-center">
                            <p className="text-sm text-muted-foreground">
                                ...
                                <br /> Comece cadastrando um administrador na equipe.
                            </p>
                            <ButtonLink variant="outline" href="/admin/equipe/administradores" className="w-full sm:w-auto">
                        ...
                        <>
                            <div className="flex flex-col gap-4">
                                <div className="flex flex-col gap-2">
                                    <Label htmlFor="adminId">Selecione o Administrador *</Label>
                                    <Controller
                                        control={controlExisting}
                                        name="adminId"
                                        render={({ field }) => (
                                            <Select value={field.value} onValueChange={field.onChange}>
                                                <SelectTrigger className="w-full bg-background p-5 h-15.5 rounded-xl" aria-invalid={errorsExisting.adminId ? "true" : "false"}>
                                                    <SelectValue placeholder="Busque um administrador..." />
                                                </SelectTrigger>
                                                <SelectContent>
                                                    {admins.length === 0 && <SelectItem value="none" disabled>Nenhum administrador encontrado</SelectItem>}
                                                    {admins.map(a => (
                                    ...
                                    />
                                    {errorsExisting.adminId && <p className="text-sm text-red-600">{errorsExisting.adminId.message}</p>}
                                </div>
                                <div className="flex flex-col gap-2">
                                    <Label htmlFor="systemRole">Nível de Acesso como Professor *</Label>
                                    <Controller
                                        control={controlExisting}
                                        name="systemRole"
                                        render={({ field }) => (
